---
name: events
description: "当用户要规划、执行、赞助、演讲或从活动获取 pipeline 时使用，包括 webinars、conference、trade show、meetup、dinner、workshop、virtual summit 和 user conference；英文触发词包括 event marketing、field marketing、webinar funnel、event ROI、badge scans、CFP、conference talk、event follow-up。覆盖 hosting、sponsoring/exhibiting、speaking、attending 四类角色。产品发布参见 launch，联合 webinar 参见 co-marketing，持续社区项目参见 community-marketing，媒体参见 public-relations，邮件序列参见 emails。 Use when the matching Chinese or English intent is present; see the named related skills for adjacent scopes."
metadata:
  version: 1.1.0
---

# 活动营销（中文执行导读）
先确定角色（主办、赞助/展出、演讲或参会）、唯一主要结果、目标 ICP、预算和团队，再先做事件组合选择，分别设计活动前、活动中和活动后的动作。
以 qualified conversations、meetings、pipeline 和 closed-won 为核心衡量，记录会话上下文并在 24–48 小时内分层跟进；把录音、复盘和证据再分发为长期内容，不把报名或 badge scans 当作结果。
